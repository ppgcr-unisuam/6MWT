...
dataset_natalia <- dataset_natalia |>
  ...
  dplyr::mutate(dplyr::across(
    c(Idade, Peso, Altura, IMC, FEV1, FC, Borg, `6MWD`),
    as.numeric
  ))
...
dataset_natalia <- dataset_natalia %>%
  dplyr::select(Corredor, Sexo, Idade, Peso, Altura, IMC, FEV1, FC, Borg, `6MWD`)
...
table1::label(dataset_natalia$FEV1) <- "%"
...
model_params <- readxl::read_xlsx(file.path(getwd(), "models.xlsx"), col_names = FALSE)
models <- paste0("[", sprintf("%02d", 1:(ncol(model_params) - 4)), "] ", model_params[1, c(-1:-4)])
colnames(model_params) <- c(
  "Variable",
  "Variable (PT)",
  "Unit",
  "Exponent",
...
available_vars <- c(available_vars, "Intercepto")  # include intercept
...
model_params_filtered <- model_params |>
  dplyr::select(
    1:4,
    all_of(model_cols[model_is_compatible])
  )

model_params_filtered <- model_params_filtered[which(model_params$`Variable (PT)` %in% available_vars), ]
...
for (b in 1:B) {
  ...
  results_ensemble$distance_meas[b] <- mean(dataset_natalia$`6MWD`, na.rm = TRUE)
  ...
  predictions_matrix <- matrix(NA, nrow = nrow(dataset_natalia), ncol = n_models_used)
  ...
  for (i in 1:n_models_used) {
    ...
    for (j in 1:nrow(dataset_natalia)) {
      pred <- 0
      for (k in 1:nrow(model_coeffs)) {
        ...
        if (var_name == "Intercepto") {
          ...
          var_value <- dataset_natalia[[var_name]][j]
          if(var_name == "Altura"){
  ...
  individual_biases <- rowMeans(predictions_matrix, na.rm = TRUE) - dataset_natalia$`6MWD`
...
dir.create(file.path(getwd(), "sim_results"), showWarnings = FALSE)
# save results
write.csv(
  results_ensemble,
  file.path(getwd(), "sim_results", "ensemble_results_natalia_oliveira_2023.csv"),
  row.names = FALSE
)
...
p_overlay <- ggplot2::ggplot() +
  ...
  ggplot2::geom_smooth(
    data = results_ensemble,
    ggplot2::aes(x = n_models, y = bias, color = "Bias trend"),
    method = "lm",
    se = FALSE,
    linewidth = 1
  ) +
...
# save tiff
ggplot2::ggsave(
  filename = file.path(getwd(), "sim_results", "Figure 1.tiff"),
  plot = p_overlay,
  device = "tiff",
  width = 10,
  height = 6,
...
write.csv(
  best_ensembles,
  file.path(getwd(), "sim_results", "best_ensembles_natalia_oliveira_2023.csv"),
  row.names = FALSE
)
...
# save tiff
ggplot2::ggsave(
  filename = file.path(getwd(), "sim_results", "Figure 2.tiff"),
  plot = p2,
  device = "tiff",
  width = 10,
  height = 6,
